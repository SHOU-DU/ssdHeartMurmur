...
class AudioClassifierFuseODconv(nn.Module):
    ...
    def __init__(self):
        ...
        self.lin = nn.Linear(in_features=135, out_features=3)
    ...
    def forward(self, x):
        ...
        xf1 = x[:, :, :64, :]  # 时频域
        xf2 = x[:, :, 64:, :]  # 时域+CST
        xf3 = torch.cat((x[:, :, 64:68, :], x[:, :, 94:, :]), dim=2)
        # outputs['xf3 shape'] = xf3.shape
        # xf2 = xf2.repeat(1, 1, 2, 1)
        # xf2 = xf2[:, :, :64, :]
        ...
        xf3 = self.ap2(xf3)
        # outputs['ap2'] = xf2.shape
        xf3 = xf3.view(xf2.shape[0], -1)
        # outputs['flatten xf2'] = xf2.shape
        # 先融合再送入DSC
        # x_fuse = self.dfm(xf1, xf2)

        x_fuse = torch.cat((xf1, xf3), dim=1)
